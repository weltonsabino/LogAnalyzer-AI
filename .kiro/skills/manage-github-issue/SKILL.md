...
description: Cria ou atualiza issues no GitHub seguindo o template padrão do projeto, usando o GitHub CLI (gh). Use quando o usuário pedir para criar, abrir, registrar, alterar ou editar uma issue no GitHub, ou quando precisar gerenciar o backlog do projeto no repositório weltonsabino/LogAnalyzer-AI.
metadata:
  author: welton
  version: "1.1"
allowed-tools:
...
Skill para criar e alterar issues no repositório `weltonsabino/LogAnalyzer-AI` usando o `gh` CLI.
...
- Sempre usar `--repo weltonsabino/LogAnalyzer-AI` em todos os comandos `gh`.
